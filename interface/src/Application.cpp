...
bool setupEssentials(int& argc, char** argv, bool runningMarkerExisted) {
    ...
    controller::StateController::setStateVariables(
        { { STATE_IN_HMD, STATE_CAMERA_FULL_SCREEN_MIRROR, STATE_CAMERA_FIRST_PERSON, STATE_CAMERA_FIRST_PERSON_LOOK_AT,
            STATE_CAMERA_THIRD_PERSON, STATE_CAMERA_ENTITY, STATE_CAMERA_INDEPENDENT, STATE_CAMERA_LOOK_AT, STATE_CAMERA_SELFIE,
            STATE_SNAP_TURN, STATE_ADVANCED_MOVEMENT_CONTROLS, STATE_GROUNDED, STATE_NAV_FOCUSED, STATE_PLATFORM_WINDOWS,
            STATE_PLATFORM_MAC, STATE_PLATFORM_ANDROID, STATE_LEFT_HAND_DOMINANT, STATE_RIGHT_HAND_DOMINANT,
            STATE_STRAFE_ENABLED } });
...
Application::Application(int& argc, char** argv, QElapsedTimer& startupTimer, bool runningMarkerExisted) :
    ...
    if (!DISABLE_WATCHDOG) {
        ...
        connect(platformHelper, &PlatformHelper::systemWillSleep, [] { DeadlockWatchdogThread::pause(); });

        connect(platformHelper, &PlatformHelper::systemWillWake, [] { DeadlockWatchdogThread::resume(); });
    ...
    DependencyManager::get<WalletScriptingInterface>()->setLimitedCommerce(isStore ||
                                                                           property(hifi::properties::STEAM).toBool());
    ...
    connect(
        myAvatar.get(), &MyAvatar::positionGoneTo, this,
        [this] {
            if (!_physicsEnabled) {
...

    {
        ...
        connect(
            scriptEngines, &ScriptEngines::scriptCountChanged, this,
            [this] {
        ...
        connect(
            scriptEngines, &ScriptEngines::scriptsReloading, this,
            [this] {
        ...
        connect(
            scriptEngines, &ScriptEngines::scriptLoadError, this,
...
// FIXME spacemouse code still needs cleanup
    ...
    auto entityScriptingInterface = DependencyManager::get<EntityScriptingInterface>();
    connect(
        entityScriptingInterface.data(), &EntityScriptingInterface::deletingEntity, this,
        ...
        if (!_autoSwitchDisplayModeSupportedHMDPlugin) {
            if (displayPlugin->isHmd() && displayPlugin->getSupportsAutoSwitch()) {
                ...
                _previousHMDWornStatus = _autoSwitchDisplayModeSupportedHMDPlugin->isDisplayVisible() &&
                                         _autoSwitchDisplayModeSupportedHMDPlugin->isActive();
            }
        }
    ...
    if (mode == CAMERA_MODE_FIRST_PERSON || mode == CAMERA_MODE_FIRST_PERSON_LOOK_AT) {
        ...
        if (isHMDMode()) {
            ...
            if (mode == CAMERA_MODE_THIRD_PERSON) {
                ...
                _myCamera.setPosition(myAvatar->getLookAtPivotPoint() + lookAtRotation * boomOffset);
...
bool Application::event(QEvent* event) {
    ...
    switch (type) {
        ...
        case QEvent::FocusOut:
            ...
            return true;
        case QEvent::FocusIn: {  //testing to see if we can set focus when focus is not set to root window.
...
bool Application::eventFilter(QObject* object, QEvent* event) {
    ...
    if (eventType == QEvent::MouseButtonPress) {
        ...
        if (mouseEvent->button() == Qt::RightButton && mouseEvent->buttons() == Qt::LeftButton &&
            ...
            QMouseEvent* newEvent = new QMouseEvent(QEvent::MouseButtonPress, mouseEvent->localPos(), mouseEvent->windowPos(),
...
NTSYSCALLAPI LONG NTAPI NtQuerySystemInformation(_In_ SYSTEM_INFORMATION_CLASS SystemInformationClass,
    ...
    _graphicsEngine.editRenderArgs([this, deltaTime](AppRenderArgs& appRenderArgs) {
        ...
        appRenderArgs._sensorToWorld = getMyAvatar()->getSensorToWorldMatrix();
        {
            ...
            appRenderArgs._renderArgs = RenderArgs(_graphicsEngine.getGPUContext(), lodManager->getVisibilityDistance(),
                                                   lodManager->getBoundaryLevelAdjust(), lodManager->getLODHalfAngleTan(),
